...
if opt.testset:
    ...
    labeled_scene_index = np.arange(128, 134)
...
for i, data in enumerate(dataloader):
    ...
    # changed road_image to float to avoid error
    ts_road_map = compute_ts_road_map(predicted_road_map, road_image.float())
    
    total_ats_bounding_boxes += ats_bounding_boxes
    total_ts_road_map += ts_road_map

    if opt.verbose:
        print(f'{i} - Bounding Box Score: {ats_bounding_boxes} - Road Map Score: {ts_road_map}')

print(f'{model_loader.team_name} - {model_loader.round_number} - Bounding Box Score: {total_ats_bounding_boxes / total} - Road Map Score: {total_ts_road_map / total}')
